class ViewSettings {
  static CLR_BK = "green"
  static CLR_BUTT = "purple"
  static CLR_SPAR = "goldenrod"
  static CLR_BK_CHART = "lightgray"
  static CLR_TEXT = "black"
  static CLR_LINE_BUTT = "purple"
  static CLR_LINE_SPAR = "goldenrod"
  static FONT_TEXT = "20px Arial"
}
...
class ChartView {
  ...
  redraw(ctx) {
    ...
    if (this.data.length > 0 && this.data[0].length > 0) {
      ...
      var txt = "Time: " + last_d.x + ", Butterflies: " + last_d.y + ", Sparrows: " + last_w.y
      ctx.fillText(txt, this.R.x + this.R.width / 2, this.R.y + this.R.height / 5);
    }

    this.draw_line(ctx, this.data[0], ViewSettings.CLR_LINE_BUTT)
    this.draw_line(ctx, this.data[1], ViewSettings.CLR_LINE_SPAR)
...
class PredPreyView {
  ...
  draw_objects(ctx, data) {
    ...
    this.draw_herd(ctx, data, Wilds.BUTT, ViewSettings.CLR_BUTT)
    this.draw_herd(ctx, data, Wilds.SPAR, ViewSettings.CLR_SPAR)
  }
}
...
class PredPreyCtrl {
  ...
  constructor(view) {
    ...
    this.socket.on('state', function(msg){
      ...
      var preds = self.state.map.filter(a => a.kind == Wilds.SPAR)
