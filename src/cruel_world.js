...
class CruelWorld extends SafeWorld {
  ...
  next() {
    ...
    this.hunted = []
    super.next()
    if (this.T == CruelWorld.SPAR_TIME)
      this.add_predator()
    ...
    var preds = this.GM.get_vals().filter(c => c.kind == Wilds.SPAR)
  ...
  hunt(pred) {
    ...
    var preys = spots.map(p => this.GM.get(p)).filter(u => u && u.kind == Wilds.BUTT)
  ...
  breed(u) {
    ...
    var preys = spots.map(p => this.GM.get(p)).filter(u => u && u.kind == Wilds.BUTT)
    var preds = spots.map(p => this.GM.get(p)).filter(u => u && u.kind == Wilds.SPAR)
